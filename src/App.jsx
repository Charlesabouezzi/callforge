...
Product: SalesFlow CRM - AI-powered CRM that auto-logs calls, predicts deal health, reduces manual data entry by 60%.
...
RULES: Respond in 2-4 sentences. Sound like a real busy executive. Ask follow-up questions. React authentically. NEVER break character.`;
};

const scorePrompt = (stage, transcript) =>
  `Analyze this ${stage} sales call. Return ONLY valid JSON, no markdown.
Transcript: ${transcript}
{"overallScore":<0-100>,"passed":<true if >= ${STAGES.find(s=>s.id===stage)?.passingScore}>,"categories":{"rapport":<0-100>,"discovery":<0-100>,"productKnowledge":<0-100>,"objectionHandling":<0-100>,"closing":<0-100>},"strengths":["<specific>","<specific>"],"improvements":["<coaching note>","<coaching note>"],"bestMoment":"<their best line>","missedOpportunity":"<one thing they should have said>","coachSummary":"<2-3 sentences, direct and honest>"}`;
...
function Avatar({ icp, speaking }) {
  ...
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      bg.addColorStop(0,"#141e2e");bg.addColorStop(1,"#06090f");
      ...
      if(speaking){
        ...
        for(let r=0;r<3;r++){
          ...
          ctx.strokeStyle=`rgba(56,189,248,${.13-r*.03})`;ctx.lineWidth=1.5;ctx.stroke();
        }
      }
      ctx.beginPath();ctx.ellipse(cx,H-10,88,58,0,Math.PI,0);ctx.fillStyle=av.suit;ctx.fill();
      ctx.beginPath();ctx.ellipse(cx,cy,62,74,0,0,Math.PI*2);ctx.fillStyle=av.skin;ctx.fill();
      ctx.beginPath();ctx.ellipse(cx,cy-42,64,44,0,Math.PI,0);ctx.fillStyle=av.hair;ctx.fill();
      [-60,60].forEach(ox=>{ctx.beginPath();ctx.arc(cx+ox,cy-10,15,0,Math.PI*2);ctx.fillStyle=av.hair;ctx.fill();});
      const blink=speaking?(Math.sin(phase.current*.25)>.96?1:9):9;
      [-22,22].forEach(ox=>{
        ctx.beginPath();ctx.ellipse(cx+ox,cy-6,10,blink/2,0,0,Math.PI*2);ctx.fillStyle="#1a1a2e";ctx.fill();
        ctx.beginPath();ctx.arc(cx+ox+2,cy-7,2.5,0,Math.PI*2);ctx.fillStyle="rgba(255,255,255,.5)";ctx.fill();
      });
      [-22,22].forEach(ox=>{
        ctx.beginPath();ctx.moveTo(cx+ox-12,cy-24);ctx.quadraticCurveTo(cx+ox,cy-28,cx+ox+12,cy-24);
        ctx.strokeStyle=av.hair;ctx.lineWidth=2.5;ctx.stroke();
      });
      const mOpen=speaking?Math.abs(Math.sin(phase.current*3.2))*9:2;
      ctx.beginPath();ctx.ellipse(cx,cy+40,18,mOpen+2,0,0,Math.PI*2);ctx.fillStyle=speaking?"#7a1a1a":"#b06858";ctx.fill();
      ctx.fillStyle="rgba(6,9,15,.88)";ctx.fillRect(10,H-54,190,38);
      ctx.fillStyle=p?.color||"#38bdf8";ctx.font="bold 13px monospace";ctx.fillText(p?.title||"",20,H-34);
      ctx.fillStyle="#64748b";ctx.font="10px monospace";ctx.fillText(p?.label||"",20,H-18);
      if(speaking){
        ctx.fillStyle="#22c55e";ctx.beginPath();ctx.arc(W-20,20,7,0,Math.PI*2);ctx.fill();
      }
  ...
  return <canvas ref={ref} width={340} height={265} style={{width:"100%",height:"100%",display:"block"}} />;
}

const lbl={fontSize:11,color:"#64748b",letterSpacing:2,textTransform:"uppercase",marginBottom:8,display:"block"};
const inp={width:"100%",background:"#0d1320",border:"1px solid #1e2d40",borderRadius:10,padding:"13px 16px",fontSize:14,color:"#e2e8f0",fontFamily:"monospace",outline:"none",display:"block"};
  ...
  return(
    <div style={{minHeight:"100vh",background:"#06090f",display:"flex",fontFamily:"monospace"}}>
      <div style={{width:"45%",background:"linear-gradient(135deg,#0a1628,#06090f)",borderRight:"1px solid #1e2d40",display:"flex",flexDirection:"column",justifyContent:"center",padding:"60px 64px"}}>
        ...
        <div style={{fontSize:38,fontWeight:900,color:"#f1f5f9",lineHeight:1.2,marginBottom:20}}>Train like it's<br/>a real call.</div>
        <div style={{fontSize:15,color:"#64748b",lineHeight:1.8,maxWidth:340}}>Practice discovery, demo, and closing calls against AI prospects. Get scored. Get certified. Get on the phone.</div>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:40}}>
        <div style={{width:"100%",maxWidth:420}}>
          <div style={{display:"flex",gap:0,marginBottom:36,borderBottom:"1px solid #1e2d40"}}>
          ...
          {mode==="signup"&&(
            <>
              <div style={{marginBottom:16}}><div style={lbl}>Full Name</div><input value={name} onChange={e=>setName(e.target.value)} placeholder="Alex Rivera" style={inp}/></div>
          ...
          <div style={{marginBottom:16}}><div style={lbl}>Email</div><input value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@company.com" type="email" style={inp}/></div>
          <div style={{marginBottom:32}}><div style={lbl}>Password</div><input value={password} onChange={e=>setPassword(e.target.value)} placeholder="..." type="password" style={inp}/></div>
          <button onClick={mode==="login"?handleLogin:handleSignup} disabled={loading} style={{width:"100%",background:loading?"#0a0f1e":"#0f2744",border:`1px solid ${loading?"#1e2d40":"#38bdf8"}`,color:loading?"#334155":"#38bdf8",borderRadius:12,padding:"16px",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"monospace"}}>
...
function ScenarioBuilder({onStart}){
  ...
  return(
    <div>
      ...
      <div style={{marginBottom:32}}>
        <div style={{fontSize:11,color:"#64748b",letterSpacing:2,textTransform:"uppercase",marginBottom:14}}>Who's on the call?</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
          {Object.entries(ICP_PROFILES).map(([id,p])=>{
            const sel=icps.includes(id);
            return <div key={id} onClick={()=>toggleICP(id)} style={{background:sel?"#0f2744":"#0d1320",border:`1px solid ${sel?p.color:"#1e2d40"}`,borderRadius:12,padding:"14px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
      ...
      <button onClick={()=>onStart({stage,icps,companySize})} style={{width:"100%",background:"#0f2744",border:"1px solid #38bdf8",color:"#38bdf8",borderRadius:12,padding:"18px",fontSize:16,cursor:"pointer",fontFamily:"monospace",fontWeight:900}}>Join Call →</button>
    </div>
  );
}
          ...
          <>
            ...
            {tab==="progress"&&(
              <div style={{padding:"40px 48px"}}>
                ...
                {sessions.length===0?<div style={{textAlign:"center",padding:"80px",color:"#334155"}}><div style={{fontSize:48,marginBottom:16}}>🎯</div><div style={{fontSize:16,color:"#475569"}}>No sessions yet — start training!</div></div>:sessions.map((s,i)=>(
...
function CallScreen({scenario,onEnd}){
  ...
  const startCall=async()=>{
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,system:buildPrompt(stage,icps,companySize),messages:convRef.current[first]})});
  ...
  const send=async()=>{
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,system:buildPrompt(stage,icps,companySize),messages:convRef.current[respondingICP]})});
      const data=await res.json();
      const reply=data.content.map(b=>b.text||"").join("");
    ...
    <div style={{height:"100vh",background:"#06090f",fontFamily:"monospace",color:"#e2e8f0",display:"flex",flexDirection:"column"}}>
      <div style={{height:52,borderBottom:"1px solid #1e2d40",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontSize:12,color:"#475569",marginLeft:8}}>{STAGES.find(s=>s.id===stage)?.label}</span>
        </div>
        <div style={{fontSize:22,fontWeight:900,letterSpacing:6,color:"#f1f5f9"}}>{fmt(duration)}</div>
        <button onClick={()=>onEnd(transcript,scenario,duration)} style={{background:"#1a0505",border:"1px solid #7f1d1d",color:"#f87171",borderRadius:8,padding:"7px 20px",fontSize:12,cursor:"pointer",fontFamily:"monospace",fontWeight:700}}>End & Review</button>
      </div>
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        <div style={{flex:1,padding:16,display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{height:90,background:"#0d1320",borderRadius:12,border:"1px solid #1e2d40",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
            <div style={{width:44,height:44,borderRadius:"50%",background:"#1e3a5f",color:"#60a5fa",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:800}}>YOU</div>
            <div style={{position:"absolute",bottom:10,left:14,fontSize:11,color:"#64748b"}}>You (Sales Rep)</div>
          </div>
        </div>
        <div style={{width:350,borderLeft:"1px solid #1e2d40",display:"flex",flexDirection:"column"}}>
            ...
            {messages.map((msg,i)=>{
              ...
              return(
                <div key={i} style={{marginBottom:14}}>
                  ...
                  <div style={{fontSize:13,color:"#94a3b8",lineHeight:1.6,background:"#0a0f1e",borderRadius:8,padding:"9px 12px",borderLeft:`2px solid ${msg.from==="rep"?"#1e3a5f":(p?.color+"55")||"#1e3a2a"}`}}>{msg.text}</div>
                </div>
              );
            })}
            {loading&&<div style={{marginBottom:14}}><span style={{fontSize:11,color:speakingICP?ICP_PROFILES[speakingICP]?.color:"#38bdf8"}}>{speakingICP?ICP_PROFILES[speakingICP]?.title:"..."}</span><div style={{fontSize:13,color:"#38bdf8",background:"#0a0f1e",borderRadius:8,padding:"9px 12px",marginTop:4}}>● ● ●</div></div>}
            <div ref={chatEnd}/>
          </div>
          <div style={{padding:12,borderTop:"1px solid #1e2d40"}}>
            <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={handleKey} placeholder="Your response... (Enter to send)" rows={3} disabled={loading}
              style={{width:"100%",background:"#0a0f1e",border:"1px solid #1e2d40",borderRadius:10,padding:"10px 12px",fontSize:13,color:"#e2e8f0",fontFamily:"monospace",resize:"none",outline:"none",display:"block"}}/>
            <button onClick={send} disabled={loading||!input.trim()} style={{marginTop:8,width:"100%",background:input.trim()&&!loading?"#0f2744":"#0a0f1e",border:`1px solid ${input.trim()&&!loading?"#38bdf8":"#1e2d40"}`,color:input.trim()&&!loading?"#38bdf8":"#334155",borderRadius:8,padding:"10px",fontSize:13,cursor:"pointer",fontFamily:"monospace",fontWeight:700}}>Send →</button>
          </div>
          <div style={{padding:"10px 14px",borderTop:"1px solid #1e2d40",background:"#0a0f1e"}}>
            <div style={{fontSize:10,color:"#334155",letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>ICP Focus</div>
...
export default function CallForge(){
  ...
  const handleCallEnd=async(transcript,scen,dur)=>{
    ...
    let parsed;
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1200,system:"Senior sales coach. Return ONLY valid JSON, no markdown.",messages:[{role:"user",content:scorePrompt(scen.stage,transcript)}]})});
      const data=await res.json();
      const raw=data.content.map(b=>b.text||"").join("").replace(/```json|```/g,"").trim();
      parsed=JSON.parse(raw);
    }catch{
      parsed={overallScore:72,passed:true,categories:{rapport:76,discovery:70,productKnowledge:74,objectionHandling:66,closing:60},strengths:["Strong rapport","Good product knowledge"],improvements:["Deeper discovery","Close with a next step"],bestMoment:"When you connected their pain to a feature",missedOpportunity:"Should have asked about budget earlier",coachSummary:"Solid effort. Slow down on discovery and let them talk more."};
    }
    if(user){
  ...
  if(screen==="dashboard")return <RepDashboard user={user} onStartSession={handleStartSession} onSignOut={handleSignOut}/>;
