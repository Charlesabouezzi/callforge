...
function LiveCallScreen({ scenario, user, onEnd }) {
  ...
  // ── RENDER ──────────────────────────────────────────────────
  return (
    <div style={{ height: "100vh", background: "#f0f9ff", fontFamily: "Inter, sans-serif", color: "#0f172a", display: "flex", flexDirection: "column" }}>
      <style>{"@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}} @keyframes spin{to{transform:rotate(360deg)}}"}</style>

      {/* TOP BAR */}
      <div style={{ height: 54, background: "#ffffff", borderBottom: "1px solid #1e2d40", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <span style={{ fontSize: 16, fontWeight: 900, color: "#38bdf8", letterSpacing: 2 }}>SALESHUB</span>
        ...
        <div style={{ fontSize: 26, fontWeight: 900, letterSpacing: 8, color: "#0f172a" }}>{fmt(duration)}</div>
        <button onClick={handleEndCall} style={{ background: "#c0392b", border: "none", color: "#fff", borderRadius: 8, padding: "10px 24px", fontSize: 13, cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 700 }}>
          End Call
        </button>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "grid", gridTemplateRows: "1fr 1fr", gap: 4, padding: 4 }}>
          ...
          {/* TOP — AI Avatar */}
          <div style={{ background: "#0a0f1a", borderRadius: 16, overflow: "hidden", border: "2px solid " + (isSpeaking ? av.color : "#bae6fd"), transition: "border-color .4s", position: "relative" }}>
          ...
          {/* BOTTOM — User camera */}
          <div style={{ background: "#f0f9ff", borderRadius: 16, overflow: "hidden", border: "2px solid " + (isListening ? "#22c55e" : "#1e3a5f"), transition: "border-color .3s", position: "relative" }}>
            ...
            {currentTranscript && (
              <div style={{ position: "absolute", bottom: 56, left: 0, right: 0, margin: "0 16px", background: "rgba(0,0,0,0.85)", borderRadius: 8, padding: "8px 14px", fontSize: 13, color: "#0f172a", fontStyle: "italic" }}>
                "{currentTranscript}"
              </div>
            )}
            ...
            <div style={{ position: "absolute", bottom: 14, right: 14, display: "flex", gap: 8 }}>
              <button onClick={toggleMic} style={{ width: 42, height: 42, borderRadius: "50%", background: micOn ? "rgba(0,0,0,0.7)" : "#7f1d1d", border: "1px solid " + (micOn ? "#94a3b8" : "#f87171"), fontSize: 18, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {micOn ? "🎙️" : "🔇"}
              </button>
              <button onClick={() => setCamOn(c => !c)} style={{ width: 42, height: 42, borderRadius: "50%", background: camOn ? "rgba(0,0,0,0.7)" : "#7f1d1d", border: "1px solid " + (camOn ? "#94a3b8" : "#f87171"), fontSize: 18, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {camOn ? "📹" : "📷"}
              </button>
            </div>
          </div>
        ...
        <div style={{ width: 320, borderLeft: "1px solid #1e2d40", display: "flex", flexDirection: "column", background: "#ffffff" }}>
          <div style={{ padding: "14px 16px", borderBottom: "1px solid #1e2d40" }}>
            <div style={{ fontSize: 10, letterSpacing: 3, color: "#64748b", textTransform: "uppercase" }}>Live Transcript</div>
            <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>Speak clearly — AI is listening</div>
          </div>
            ...
            {messages.map((msg, i) => (
              <div key={i} style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 10, color: "#94a3b8" }}>{msg.ts}</span>
                </div>
                <div style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.6, background: "#ffffff", borderRadius: 8, padding: "10px 12px", borderLeft: "2px solid " + (msg.from === "rep" ? "#1e3a5f" : av.color + "55") }}>
                  {msg.text}
                </div>
              </div>
            ))}
          ...
          {/* Coaching tips */}
          <div style={{ padding: "12px 16px", borderTop: "1px solid #1e2d40", background: "#f8fafc" }}>
            <div style={{ fontSize: 10, color: "#94a3b8", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>Focus Areas</div>
          ...
          <div style={{ padding: "12px 16px", borderTop: "1px solid #1e2d40" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, justifyContent: "center" }}>
              <div style={{ width: 10, height: 10, borderRadius: "50%", background: isListening ? "#22c55e" : isSpeaking ? av.color : "#94a3b8", animation: (isListening || isSpeaking) ? "pulse 1s infinite" : "none" }} />
              <span style={{ fontSize: 12, color: isListening ? "#22c55e" : isSpeaking ? av.color : "#64748b" }}>
...
function Debrief({ score, scenario, duration, onRetry, onDashboard }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f9ff", fontFamily: "Inter, sans-serif", color: "#0f172a", padding: "48px 40px" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          ...
          <div style={{ fontSize: 13, color: "#64748b", marginTop: 6 }}>{fmt(duration)} · {av.title} · {COMPANY_SIZES[scenario.companySize]}</div>
        </div>

        <div style={{ background: "#ffffff", border: "1px solid #1e2d40", borderRadius: 14, padding: "22px 28px", marginBottom: 24 }}>
          <div style={{ fontSize: 11, letterSpacing: 3, color: "#38bdf8", textTransform: "uppercase", marginBottom: 10 }}>Coach's Take</div>
          <div style={{ fontSize: 14, color: "#1e293b", lineHeight: 1.8 }}>{score.coachSummary}</div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 24 }}>
          {Object.keys(score.categories || {}).map(k => {
            ...
            return (
              <div key={k} style={{ background: "#ffffff", border: "1px solid #1e2d40", borderRadius: 12, padding: "16px 18px" }}>
                <div style={{ fontSize: 11, color: "#64748b", textTransform: "capitalize", marginBottom: 8 }}>{k.replace(/([A-Z])/g, " $1")}</div>
                <div style={{ background: "#f8fafc", borderRadius: 4, height: 4, marginBottom: 6 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>
          <div style={{ background: "#f0f9ff", border: "1px solid #1e3a5f", borderRadius: 12, padding: "18px 20px" }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <button onClick={onRetry} style={{ flex: 1, background: "#ffffff", border: "1px solid #1e2d40", color: "#94a3b8", borderRadius: 10, padding: "14px", fontSize: 13, cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 600 }}>Retry</button>
          <button onClick={onDashboard} style={{ flex: 2, background: "#0ea5e9", border: "1px solid #38bdf8", color: "#38bdf8", borderRadius: 10, padding: "14px", fontSize: 14, cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 800 }}>Back to Dashboard</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          {STAGES.map(s => (
            <div key={s.id} onClick={() => setStage(s.id)} style={{ flex: 1, background: stage === s.id ? "#0ea5e9" : "#ffffff", border: "1px solid " + (stage === s.id ? s.color : "#bae6fd"), borderRadius: 14, padding: "20px 18px", cursor: "pointer", textAlign: "center" }}>
              <div style={{ fontSize: 26, marginBottom: 8 }}>{s.icon}</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: stage === s.id ? s.color : "#1e293b" }}>{s.label}</div>
              <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>{s.duration}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
          {Object.keys(ICP_AVATARS).map(id => {
            ...
            const sel = icp === id;
            return (
              <div key={id} onClick={() => setIcp(id)} style={{ background: sel ? "#0ea5e9" : "#ffffff", border: "1px solid " + (sel ? av.color : "#bae6fd"), borderRadius: 14, padding: "18px 16px", cursor: "pointer", textAlign: "center" }}>
                <div style={{ width: 48, height: 48, borderRadius: "50%", background: av.color + "22", border: "2px solid " + (sel ? av.color : "#94a3b8"), display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, color: av.color, margin: "0 auto 10px" }}>
                  {av.name.split(" ").map(n => n[0]).join("")}
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: sel ? av.color : "#0f172a" }}>{av.name}</div>
                <div style={{ fontSize: 11, color: av.color, marginTop: 2 }}>{av.title}</div>
                <div style={{ fontSize: 10, color: "#64748b", marginTop: 4 }}>{av.focus.split(" ").slice(0, 4).join(" ")}</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{ marginBottom: 36 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {Object.keys(COMPANY_SIZES).map(id => (
            <div key={id} onClick={() => setCompanySize(id)} style={{ background: companySize === id ? "#0ea5e9" : "#ffffff", border: "1px solid " + (companySize === id ? "#38bdf8" : "#bae6fd"), borderRadius: 8, padding: "9px 16px", cursor: "pointer", fontSize: 12, color: companySize === id ? "#38bdf8" : "#64748b", fontWeight: companySize === id ? 700 : 400 }}>
              {COMPANY_SIZES[id]}
            </div>
          ))}
        </div>
      ...
      <div style={{ background: "#ffffff", border: "1px solid #1e2d40", borderRadius: 12, padding: "18px 22px", marginBottom: 28 }}>
        ...
        <div style={{ fontSize: 12, color: "#64748b", marginTop: 4 }}>{COMPANY_SIZES[companySize]}</div>
      </div>

      <div style={{ background: "#f0f9ff", border: "1px solid #1e3a5f", borderRadius: 12, padding: "14px 20px", marginBottom: 24 }}>
        <div style={{ fontSize: 12, color: "#60a5fa" }}>🎙️ Voice-enabled call — speak naturally, the AI responds in real time</div>
      </div>

      <button onClick={() => onStart({ stage, icp, companySize })} style={{ width: "100%", background: "#0ea5e9", border: "1px solid #38bdf8", color: "#38bdf8", borderRadius: 12, padding: "18px", fontSize: 16, cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 900 }}>
        Join Call →
      </button>
    </div>
  );
...
function LoginScreen({ onAuth }) {
  ...
  const resetPassword = async () => {
    ...
    await supabase.auth.resetPasswordForEmail(email, { redirectTo: "https://saleshub.tech" });
    setMsg("Password reset email sent! Check your inbox.");
  };

  const inp = { width: "100%", background: "#ffffff", border: "1px solid #1e2d40", borderRadius: 10, padding: "13px 16px", fontSize: 14, color: "#0f172a", fontFamily: "Inter, sans-serif", outline: "none", display: "block", marginBottom: 14 };

  return (
    <div style={{ minHeight: "100vh", background: "#f0f9ff", display: "flex", fontFamily: "Inter, sans-serif" }}>
      <div style={{ width: "42%", background: "linear-gradient(135deg,#0a1628,#06090f)", borderRight: "1px solid #1e2d40", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px" }}>
        <div style={{ fontSize: 26, fontWeight: 900, color: "#38bdf8", letterSpacing: 4, marginBottom: 6 }}>SALESHUB</div>
        <div style={{ fontSize: 12, color: "#64748b", letterSpacing: 2, marginBottom: 60, textTransform: "uppercase" }}>AI Sales Training Platform</div>
        <div style={{ fontSize: 36, fontWeight: 900, color: "#0f172a", lineHeight: 1.2, marginBottom: 18 }}>Train like it's<br />a real call.</div>
        ...
        <div style={{ width: "100%", maxWidth: 400 }}>
          <div style={{ display: "flex", marginBottom: 32, borderBottom: "1px solid #1e2d40" }}>
            {[["login", "Sign In"], ["signup", "Create Account"]].map(item => (
              <div key={item[0]} onClick={() => { setMode(item[0]); setErr(""); setMsg(""); }} style={{ flex: 1, textAlign: "center", padding: "12px", fontSize: 13, cursor: "pointer", color: mode === item[0] ? "#38bdf8" : "#64748b", borderBottom: mode === item[0] ? "2px solid #38bdf8" : "2px solid transparent", marginBottom: -1 }}>{item[1]}</div>
            ))}
          </div>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
                {[["rep", "Sales Rep"], ["manager", "Manager"]].map(item => (
                  <div key={item[0]} onClick={() => setRole(item[0])} style={{ background: role === item[0] ? "#0ea5e9" : "#ffffff", border: "1px solid " + (role === item[0] ? "#38bdf8" : "#bae6fd"), borderRadius: 10, padding: "12px", textAlign: "center", cursor: "pointer", fontSize: 13, color: role === item[0] ? "#38bdf8" : "#64748b", fontWeight: role === item[0] ? 700 : 400 }}>{item[1]}</div>
                ))}
              </div>
            </div>
          )}
          ...
          <button onClick={mode === "login" ? login : signup} disabled={loading} style={{ width: "100%", background: "#0ea5e9", border: "1px solid #38bdf8", color: "#38bdf8", borderRadius: 12, padding: "16px", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "Inter, sans-serif", marginBottom: 12 }}>
            {loading ? "Please wait..." : mode === "login" ? "Sign In →" : "Create Account →"}
          </button>
          {mode === "login" && (
            <div onClick={resetPassword} style={{ textAlign: "center", fontSize: 12, color: "#64748b", cursor: "pointer", textDecoration: "underline" }}>Forgot password?</div>
          )}
        </div>
      </div>
    </div>
...
function Dashboard({ user, onStartSession, onSignOut }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f9ff", fontFamily: "Inter, sans-serif", color: "#0f172a", display: "flex" }}>
      {/* Sidebar */}
      <div style={{ width: 220, background: "#f8fafc", borderRight: "1px solid #1e2d40", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "22px 20px", borderBottom: "1px solid #1e2d40" }}>
          <div style={{ fontSize: 16, fontWeight: 900, color: "#38bdf8", letterSpacing: 3 }}>SALESHUB</div>
          <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 2, letterSpacing: 2 }}>AI SALES TRAINING</div>
        </div>
        <nav style={{ padding: "16px 0", flex: 1 }}>
          {[["home", "🏠", "Dashboard"], ["train", "🎯", "Train"], ["missions", "📤", "Missions" + (unread ? " (" + unread + ")" : "")], ["progress", "📊", "My Progress"]].map(item => (
            <div key={item[0]} onClick={() => setTab(item[0])} style={{ padding: "11px 20px", fontSize: 13, color: tab === item[0] ? "#38bdf8" : "#64748b", cursor: "pointer", borderLeft: "2px solid " + (tab === item[0] ? "#38bdf8" : "transparent"), background: tab === item[0] ? "#0ea5e9" : "transparent", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid #1e2d40" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 12, color: "#1e293b", fontWeight: 600 }}>{user.name}</div>
              <div style={{ fontSize: 10, color: "#64748b" }}>{user.role === "manager" ? "Manager" : "Sales Rep"}</div>
            </div>
          </div>
          <div onClick={onSignOut} style={{ fontSize: 11, color: "#64748b", cursor: "pointer" }}>Sign Out</div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {loading ? (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "#64748b" }}>Loading...</div>
        ) : (
          <div>
            {tab === "home" && (
              <div style={{ padding: "40px 48px" }}>
                <div style={{ marginBottom: 40 }}>
                  ...
                  <div style={{ fontSize: 32, fontWeight: 900, color: "#0f172a" }}>Hey, {user.name ? user.name.split(" ")[0] : "there"} 👋</div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 40 }}>
                  {[["Avg Score", avgScore || "—", avgScore ? sc(avgScore) : "#64748b"], ["Sessions", sessions.length, "#a78bfa"], ["Passed", sessions.filter(s => s.passed).length, "#22c55e"], ["Missions", missions.length, "#f59e0b"]].map(item => (
                    <div key={item[0]} style={{ background: "#ffffff", border: "1px solid #1e2d40", borderRadius: 14, padding: "20px 22px" }}>
                ...
                {missions.filter(m => !m.completed).length > 0 && (
                  <div style={{ marginBottom: 40 }}>
                    ...
                    {missions.filter(m => !m.completed).map(m => {
                      ...
                      return (
                        <div key={m.id} onClick={() => setTab("missions")} style={{ background: "#f0f9ff", border: "1px solid " + (m.read ? "#bae6fd" : "#38bdf8"), borderRadius: 14, padding: "20px 24px", marginBottom: 12, cursor: "pointer", position: "relative" }}>
                          {!m.read && <div style={{ position: "absolute", top: 16, right: 16, width: 8, height: 8, borderRadius: "50%", background: "#38bdf8" }} />}
                          <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", marginBottom: 8 }}>{stageInfo.icon} {stageInfo.label}</div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
                  {STAGES.map(stage => (
                    <div key={stage.id} onClick={() => setTab("train")} style={{ background: "#ffffff", border: "1px solid #1e2d40", borderRadius: 14, padding: "22px 20px", cursor: "pointer" }}>
                      <div style={{ fontSize: 26, marginBottom: 10 }}>{stage.icon}</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", marginBottom: 4 }}>{stage.label}</div>
            ...
            {tab === "train" && (
              <div style={{ padding: "40px 48px" }}>
                <div style={{ marginBottom: 40 }}>
                  ...
                  <div style={{ fontSize: 28, fontWeight: 900, color: "#0f172a" }}>Build Your Scenario</div>
            ...
            {tab === "missions" && (
              <div style={{ padding: "40px 48px" }}>
                <div style={{ marginBottom: 36 }}>
                  <div style={{ fontSize: 28, fontWeight: 900, color: "#0f172a" }}>Manager Missions</div>
                </div>
                {missions.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "60px", color: "#94a3b8" }}>No missions yet</div>
                ) : missions.map(m => {
                  const stageInfo = STAGES.find(s => s.id === m.stage) || {};
                  return (
                    <div key={m.id} style={{ background: "#ffffff", border: "1px solid " + (m.read ? "#bae6fd" : "#38bdf8"), borderRadius: 16, padding: "24px 26px", marginBottom: 16 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
                        <div style={{ fontSize: 16, fontWeight: 800, color: "#0f172a" }}>{stageInfo.icon} {stageInfo.label}</div>
                        {m.completed ? <div style={{ background: "#0a1a0a", border: "1px solid #1a3a1a", borderRadius: 8, padding: "6px 14px", fontSize: 12, color: "#22c55e" }}>Done</div> : <div style={{ background: "#f0f9ff", border: "1px solid #1e3a5f", borderRadius: 8, padding: "6px 14px", fontSize: 12, color: "#38bdf8" }}>Pending</div>}
                      </div>
                      {m.note && <div style={{ background: "#f8fafc", borderRadius: 10, padding: "12px 16px", marginBottom: 14, fontSize: 13, color: "#94a3b8", fontStyle: "italic" }}>"{m.note}"</div>}
                      {!m.completed && (
                        <button onClick={() => onStartSession({ stage: m.stage, icp: m.icps && m.icps[0] ? m.icps[0] : "vp_sales", companySize: m.company_size || "smb", missionId: m.id })}
                          style={{ background: "#0ea5e9", border: "1px solid #38bdf8", color: "#38bdf8", borderRadius: 10, padding: "12px 24px", fontSize: 13, cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 700 }}>
                          Start Mission →
                        </button>
                      )}
                    </div>
            ...
            {tab === "progress" && (
              <div style={{ padding: "40px 48px" }}>
                <div style={{ marginBottom: 40 }}>
                  <div style={{ fontSize: 28, fontWeight: 900, color: "#0f172a" }}>My Progress</div>
                </div>
                {sessions.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "80px", color: "#94a3b8" }}>
                    <div style={{ fontSize: 48, marginBottom: 16 }}>🎯</div>
                    <div style={{ fontSize: 16, color: "#64748b" }}>No sessions yet — start training!</div>
                  </div>
                ) : sessions.map((s, i) => {
                  const stageInfo = STAGES.find(st => st.id === s.stage) || {};
                  const av = s.icp ? ICP_AVATARS[s.icp] : null;
                  return (
                    <div key={i} style={{ background: "#ffffff", border: "1px solid #1e2d40", borderRadius: 12, padding: "18px 22px", marginBottom: 12, display: "flex", alignItems: "center", gap: 16 }}>
                      <div style={{ fontSize: 24 }}>{stageInfo.icon}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a" }}>{stageInfo.label}</div>
                        <div style={{ fontSize: 11, color: "#64748b" }}>{av ? av.name + " · " + av.title : ""} · {COMPANY_SIZES[s.company_size]}</div>
                        <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 2 }}>{new Date(s.created_at).toLocaleDateString()}</div>
                      </div>
...
export default function SalesHub() {
  ...
  if (screen === "loading") return (
    <div style={{ minHeight: "100vh", background: "#f0f9ff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter, sans-serif" }}>
      <style>{"@keyframes spin{to{transform:rotate(360deg)}}"}</style>
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: 28, fontWeight: 900, color: "#38bdf8", letterSpacing: 4, marginBottom: 20 }}>SALESHUB</div>
  ...
  if (screen === "analyzing") return (
    <div style={{ minHeight: "100vh", background: "#f0f9ff", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "Inter, sans-serif" }}>
      <style>{"@keyframes spin{to{transform:rotate(360deg)}}"}</style>
